...
class Encoder:
    ...
    def template_textual_encoding_df(self, template) -> list[str]:
        return self.merged_data.progress_apply(
            lambda row: self.basic_format_df_row(row, template), axis=1
        ).tolist()
    ...
    def template_textual_encoding_dict(self, template) -> list[str]:
        return [
            self.basic_format_dict(d, template)
            for d in tqdm(self.merged_data, desc="Encoding dictionaries")
        ]
